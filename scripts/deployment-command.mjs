...
function readTargetEnvironment(root, environment, envFiles) {
 ...
 * @param {string[]} argv
 */
export function prepareDeployment(root, argv) {
  ...
  if (!values.help) {
    ...
    const environment = readTargetEnvironment(root, values.env, envFiles.length ? envFiles : undefined);
    const config = unstable_readConfig({
      config: resolve(root, 'wrangler.jsonc'),
      env: values.env ?? environment.CLOUDFLARE_ENV?.value,
    }, { hideWarnings: true });
    name = environment.WRANGLER_CI_OVERRIDE_NAME?.value ?? values.name ?? config.name;
